...
void Pdw::Detect()
{
	cv::resize(frame, frame, cv::Size(), 0.5, 0.5);//0.5
	Mat src, display;
	IntImage<double> original;
	result_out.clear();
	frame(cv::Rect(110, 90, 400, 190)).copyTo(src);//左边，上边，宽度，高度

	line(src, Point(0, 200), Point(205, 0), cv::Scalar(0, 255, 0), 1);
	line(src, Point(399, 197), Point(195, 0), cv::Scalar(0, 255, 0), 1);
	imshow("process", src);

	//std::cout << src.dims << std::endl;
	...
	for (size_t i = 0; i < results.size(); i++)
	{
		int width, height;
		int xCen, yCen;
		width = abs(results[i].right - results[i].left);
		height = abs(results[i].top - results[i].bottom);
		if (height > 115 && width > 30)
		{
			src(cv::Rect(results[i].left, results[i].top + (height * 0.75), width, height - (height * 0.75))).copyTo(display);
			cv::cvtColor(display, display, CV_RGB2GRAY);//把图片转化为灰度图
			threshold(display, display, 150, 255, CV_THRESH_BINARY);//200
			nonZerosNum = countNonZero(display);
			Rect tempRect;
			xCen = results[i].left + width / 2;
			yCen = results[i].top + height / 2;
			if ( ( nonZerosNum > 5) && ( 0.974 * xCen - yCen - 200 ) < 0 && ( 189 - 0.969 * xCen - yCen ) < 0 )
			{
				curWarnFlag = 0x01;
				results[i].left += 110;
				results[i].top += 90;
				...
				//cv::rectangle(frame, cvPoint(results[i].left + 110, results[i].top + 90),
					//cvPoint(results[i].right + 110, results[i].bottom + 90), cv::Scalar(0, 255, 0), 2);
				results[i].left *= 2;
				results[i].top *= 2;
				results[i].right *= 2;
				results[i].bottom *= 2;
				
				tempRect.x = results[i].left;
				tempRect.y = results[i].top;
				tempRect.width = abs(results[i].right - results[i].left);
				tempRect.height = abs(results[i].top - results[i].bottom);

				result_out.push_back(tempRect);
	...
	if (curWarnFlag == 0x01 && preWarnFlag == 0x01)
	{
		warnFlag = 0x01;
	}

	preWarnFlag = curWarnFlag;
}